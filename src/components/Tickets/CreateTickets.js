	...
	const createTicketHandler = async (e) => {
		...
		try {
			...
			for (let i = 0; i < unitInput; i++) {
				batch.set(doc(db, 'tickets', addLeadingZeros(++ticketsCount, totalDigits)), {
		...
		updateCouponImage(`${addLeadingZeros(ticketsCount - unitInput, totalDigits)} - ${addLeadingZeros(ticketsCount, totalDigits)}`);
		resetInputs();
		setLoading(false);
	}
		...
		<div className="uk-container">
			<div className="uk-card uk-card-body uk-card-default">
				...
				<form onSubmit={createTicketHandler}>
					...
					<a ref={whatsappButton} href="/" target="_blank" className="uk-flex whatsapp-button uk-hidden" onClick={() => whatsappButton.current.classList.add('uk-hidden')} type="button" tabIndex="6">
